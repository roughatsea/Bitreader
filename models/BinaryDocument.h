...
public: // Expose the following constructors or accessors to callers such as the parser and printer.
    ...
    const std::vector<Record>& getRecords() const { return records_; } // Return a read-only reference to the collection of complete records assembled by readDocument, avoiding a copy when the parser, printer, or tests traverse the hierarchy.
    void setRecords(std::vector<Record> value) { records_ = std::move(value); } // Move the supplied vector into the collection of complete records assembled by readDocument; the corresponding getter above exposes it to callers.
...
private: // Restrict direct access to the state below; callers use the public methods above.
    ...
    std::vector<Record> records_; // Own storage for the collection of complete records assembled by readDocument; its vector starts empty until setRecords receives the parsed collection.
