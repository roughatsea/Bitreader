...
{ // Begin readBits's body; the following statements implement the declaration immediately above.
    if (count == 0 || count > 64) { // Reject widths outside the result type's capacity before consuming any input. Angle syntax: The > comparison rejects counts above sixty-four because the returned uint64_t cannot hold a wider field.
    ...
    } // End the branch guarded by count == 0 || count > 64; subsequent lines belong to its enclosing scope. Angle syntax: The > in the existing comment repeats the maximum-width guard above; it identifies the validation branch ending here and performs no new comparison.

    std::uint64_t value = 0; // Initialize the field accumulator that receives each extracted bit below.
    for (unsigned i = 0; i < count; ++i) { // Extract exactly count bits, fetching additional bytes only when needed. Angle syntax: The < comparison keeps the zero-based extraction index below count, so the loop runs exactly count times and consumes no extra bit.
        if (remainingBits_ == 0) { // Refill the byte cache only when all bits from its previous byte have been consumed.
            ...
            currentByte_ = static_cast<std::uint8_t>(static_cast<unsigned char>(byte)); // Preserve the raw eight-bit pattern even when char is signed, then store it in the cache. Angle syntax: The <std::uint8_t> names the explicit conversion's target type; this stores the converted byte in the reader's fixed-width eight-bit cache. The <unsigned char> names the explicit conversion's target type; this preserves the fetched byte's unsigned bit pattern before it is cached.
        ...
        value = (value << 1) | ((currentByte_ >> remainingBits_) & 1u); // Shift earlier field bits left and append the selected cached bit to the least significant position. Angle syntax: The integer << 1 shifts the accumulated field left by one position to make space for the next bit; it is a bitwise shift, unlike the stream insertion operators used by the printer. The integer >> remainingBits_ moves the next cached bit down to position zero; the following & 1u masks away other bits before that bit is appended to value.
